...
class Admin extends Model {
    ...
    public function search($params) {
        $query = $this->db->prepare("INSERT INTO `settings` (`name`, `value`) VALUES('search_api_key', ?), ('web_per_page', ?), ('images_per_page', ?), ('videos_per_page', ?), ('news_per_page', ?), ('search_sites', ?), ('search_market', ?), ('search_per_ip', ?), ('suggestions_per_ip', ?), ('search_answers', ?), ('search_suggestions', ?), ('search_related', ?) ON DUPLICATE KEY UPDATE `name` = VALUES(`name`), `value` = VALUES(`value`)");
        $query->bind_param('ssssssssssss', $params['search_api_key'], $params['web_per_page'], $params['images_per_page'], $params['videos_per_page'], $params['news_per_page'], $params['search_sites'], $params['search_market'], $params['search_per_ip'], $params['suggestions_per_ip'], $params['search_answers'], $params['search_suggestions'], $params['search_related']);
        $query->execute();
        $query->close();
    }
