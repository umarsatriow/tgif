...
class Web extends Controller {
    ...
    public function index() {
        ...
        // Market
        $params['mkt'] = (isset($_COOKIE['market']) && in_array($_COOKIE['market'], array_keys($search->getMarkets())) ? $_COOKIE['market'] : $this->settings['search_market']);
        ...
        if($this->settings['ads_safe'] == 1 && $params['safeSearch'] !== 'Strict') {
            $data['show_ads'] = false;
        } else {
            $data['show_ads'] = true;
        }
     ...
     * @return  string
     */
    private function iaMap($value) {
        $data['result'] = !empty($value[1]) ? $value[1] : $value[2];
        return $this->view->render($data, 'web/ia/map');
    }
