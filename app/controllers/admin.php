    ...
    public function general() {
        ...
        if(isset($_POST['submit'])) {
            ...
            if(isset($_FILES['site_logo']) && !empty($_FILES['site_logo']['name'])) {
                ...
                if($_FILES['site_logo']['error'] == 0 && pathinfo($_FILES['site_logo']['name'], PATHINFO_EXTENSION) == 'png') {
                    // If the file can't be written on the disk (will return 0)
                    if(move_uploaded_file($_FILES['site_logo']['tmp_name'], sprintf('%s/../../%s/%s/%s/assets/images/logo.png', __DIR__, PUBLIC_PATH, THEME_PATH, $this->settings['site_theme'])) == false) {
                        $_SESSION['message'][] = ['error', sprintf($this->lang['upload_error_code'], $_FILES['site_logo']['error'])];
                    }
                } else {
                    $_SESSION['message'][] = ['error', sprintf($this->lang['upload_error_code'], $_FILES['site_logo']['error'])];
                }
