...
<div class="site-result">
    ...
    <div class="site-description"><?=$data['results']['snippet']?></div>
    ...
    <?php if(isset($data['results']['deepLinks'])): ?>
        <div class="<?php if(array_key_exists_r('snippet', $data['results']['deepLinks']) == true): ?>deep-links<?php else: ?>deep-links-inline<?php endif ?>">
            <?php for($i = 0, $n = count($data['results']['deepLinks']), $t = array_key_exists_r('snippet', $data['results']['deepLinks']); $i < $n; $i++): ?>
                ...
                <?php endif ?>
                <div class="deep-link">
                    <div class="site-title"><a href="<?=$data['results']['deepLinks'][$i]['url']?>"<?=($data['cookie']['new_window'] > 0 ? ' target="_blank"' : '')?> data-nd><?=$data['results']['deepLinks'][$i]['name']?></a><?php if(isset($data['results']['deepLinks'][$i]['snippet'])): ?><span class="site-description"><?=str_limit($data['results']['deepLinks'][$i]['snippet'], 75)?></span><?php endif ?></div>
                </div>
